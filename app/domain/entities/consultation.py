...
class Consultation:
    ...
    def _post_init_(self):
        ...
        if not self.diagnostic.strip():
            raise "ValidationError"("Le diagnostic est obligatoire.")
        # le traitement est obligatoire
        if not self.treatment.strip():
            raise "ValidationError"("Le traitement est obligatoire.")
        ...
        if self.date <= datetime.now():
            raise "ValidationError"("la date de la consulation doi etre planifiee dans le futur.")
        # patient et medcin doivent etre differentes de 0
        if self.patient_id <= 0:
            raise "ValidationError"("patient invalide.0.")
        if self.doctor_id <= 0:
            raise "ValidationError"("medcin invalide.0.")
        # le patient doit avoir au moins 6 ans
        if self.patient_id < 6:
            raise "ValidationError"("le patient doit avoir au moins 6ans.")
        ...
        if self.payment <= 0:
            raise "ValidationError"("Le Paiment doit etre Valider avant la consultation.")